...
public class PopulateGrid_Drums : MonoBehaviour
{
    ...
    public void PopulateDrumGridFunction(int numberToPass) {
        rowCount = -1;
        stepCellsTotal = -1;
    ...
    public void ReColorGridFunction() {
        GameObject[] whitecells = GameObject.FindGameObjectsWithTag("drum_cell_white");
        foreach(GameObject cell in whitecells) { 
            cell.GetComponent<RawImage>().color = new Color(0.7f, 0.7f, 0.7f); 
        }  

        GameObject[] blackcells = GameObject.FindGameObjectsWithTag("drum_cell_black");
