...
public class Keyboard : MonoBehaviour
{
    ...
    private void registerEvents(GameObject note){
        ...
        pointerDown.callback.AddListener((e) => keyOn(note.GetComponent<PianoTile>().midiNote, note));
    ...
    public void keyOn(int midiNumber, GameObject note){
        Debug.Log("Clicked " + midiNumber); 
        GameObject.Find("HelmController").GetComponent<AudioHelm.HelmController>().NoteOn(midiNumber);
        var tempColor = note.gameObject.GetComponent<Image>().color;
        tempColor.a = 0.5f;
        note.gameObject.GetComponent<Image>().color = tempColor;
    }

    public void keyOff(int midiNumber, GameObject note){
        Debug.Log("Released " + midiNumber);
        GameObject.Find("HelmController").GetComponent<AudioHelm.HelmController>().NoteOff(midiNumber);
